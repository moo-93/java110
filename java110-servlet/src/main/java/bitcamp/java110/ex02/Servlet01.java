...
// 1) 웹 애플리케이션의 경로를 root(/)로 설정 했을 경우
//      http://localhost:8888/ex01/servlet01
// 2) 경로를 web01로 설정했을 경우,
...
public class Servlet01 implements Servlet{
    ...
    public ServletConfig getServletConfig() {
        ...
        // 그래서 이 메서드는 init()에서 파라미터로 받은 ServletConfig객체를 리턴해야한다.
        return this.config;
    }
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        ...
        // 예를 들면, 게시물 목록을 출력하거나 로그인을 처리하거나 회원 가입을 처리하는 등...
    ...
    public void destroy() {
        ...
        // 이 메서드에서는 init()에서 준비한 자원을 해제시키는 일을 한다.
...
// - 해당 서블릿의 인스턴스가 이미 생성되었다면 service()를 호출한다.
// - 해당 서블릿의 인스턴스가 아직 생성되지 않았다면
//      - 서블릿 객체 생성 및 생성자 호출
//      - init() 호출
//      - service() 호출
// - 만약 서블릿 컨테이너를 종료하거나 웹 애플리케이션의 실행을 멈춘다면
//      - 생성된 모든 서블릿들에 대해 destory() 호출
...
//   why? 서블릿 객체는 한 개만 생성되고 모든 클라이언트가 공유한다.