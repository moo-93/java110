...
public class Servlet06 extends GenericServlet{
    ...
    public void service(
        ...
        // 주의!
        // => GET 요청할 때 URL에 한글이 포함될 경우 따로 POST 처럼 설정하지 않아도 된다. 
        // => 그런데 톰캣 7 이하에서는 server.xml에 커넥터 부분에 
...
// 7비트 장비 ?
// => 오래전에 구축된 일부 네트워크 장비(Gateway 등) 7비트인 경우가 있다.
...
//    원래 ASCII 문자는 그대로 두고 한글과 같은 7비트를 초과하는 코드인경우
//    URL 인코딩을 진행한다.
// => 웹서버는 웹 브라우저로 부터 받은 URL 인코딩 값을 디코딩 하여 원래의 바이트 값으로 바꾼다.
//    따라서 개발자가 개입할 필요가 없다.
// 그런데 왜 우리가 URL 인코딩 /디코딩을 알아야 함?
// => 웹 브라우저와 웹서버 사이에 데이터를 주고받는 HTTP 프로토콜 분석하다 보면
//    %xx5xx%xx 간은데이터를 ㅂ게 된다.
//    이런경우 당황하지 말고 정확히 이해하라고 설명한 것이다.
// => 웹브라우저를 통해 서버에 데이터를 보내는 것이 아니라
// => 개발자가 직접 소켓 프로그래밍을 통해 데이터를 보낼 때는
//    URL 인코딩이 자동으로 처리되는 것이 아니기 때문에
//    개발자가 직접  java-net URIencoder를 사용하여 직접 URL 인코딩 해야 한다.
