/*
 * 리다이렉트
 * => 서버가 컨텐트를 보내지 않고, 즉 클라이언트가 뭔가를 출력하지 않고
...
public class Servlet02 extends HttpServlet{
    ...
    public void doGet(
        ...
        out.println("</head>");
        out.println("<body>");
        out.println("<h1>Server02 실행!</h1>");
        out.println("</body>");
        out.println("</html>");
        
        // sendRedirect()를 호출하기 전에 버퍼의 내용을 웹 브라우저로 강제 출력한다면
        // => sendRedirect()는 무시된다.
        // out.flush();
        ...
        // 질문 1) 그럼 지금까지 출력한 내용은 어떻게 되나여?
        // => out.println() 등을 사용하여 출력한 내용은
        //    실제 내부 버퍼에 보관되어 있다.
        //    즉 아직 출력하지 않은 상태이다.
        // => sendRedirect를 호출하면 이 버퍼에 보관된 내용을 버려버린다.
        // => 응답할 때도 웹 브라우저에게 message body를 출력하지 않는다.
        //
        // 질문2) 버퍼가 꽉차면 어떻게 되나요?
        // => 그럼 자동으로 출력한다.
        // => 따라서 출력했기 때문에 sendRedirect는 무시된다. (출력이 안되거나 에러가 뜨거나..)
    }    
}
...
Location: http://google.com <==== 웹 브라우저가 다시 요청할 URL 정보이다.
Content-Type: text/html;charset=UTF-8
Content-Length: 0
Date: Fri, 28 Sep 2018 05:30:15 GMT
