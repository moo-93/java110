...
public class MyWebApplicationInitalizer 
    /*implements WebApplicationInitializer*/{

    public void onStartup(ServletContext servletContext) throws ServletException {
        // 구동 원리:
        // 1) 서블릿 컨테이너(예: 톰캣 서버)를 실행한다.
        // 2) 각각의 웹 애플리케이션의 /WEB-INF/lib 폴더에 있는 .jar 파일을 준비한다.
        ...
        // 4) ServletContainerInitalizer 를 실행하는 과정
        //    => spring-web-xxx.jar 파일에 해당 구현체가 있다.
        //    => org.springframework.web.SpringServletConteainerInitalizer
        //    => 서블릿 컨테이너는 웹 애플리케이션을 시작할 때
        //       이 클래스에 대해 다음 멧드를 호출한다.
        //
        //       onStartup(Set<Class<?>> c, ServletContext ctx)
        //
        //    => 이 메서드의 첫 번째 파라미터로 받는 것은 
        //       SpringServletConteainerInitalizer 클래스 위에 선언한
        //       @HandlesTypes 에노테이션에 지정된
        //       지정된 WebApplicationInitalizer.class 클래스의 구현체 목록이다.
        //    => 즉 WebApplicationInitalizer를 구현한 클래스를 모두 찾아
        //       그 클래스 정보를 목록에 담아 준다.
        //    => 그러면 SpringServletContainerInitializer의 onStart()에서는
        //       목록에 등록된 클래스의 객체를 한 개 씩 만들고,
        //       그 객체에 대해 onStartup()을 호출한다.
        //    => 즉 지금 여기에서 만드는 MYWebApplicationInitilalizer
