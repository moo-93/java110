/* 세션 다루기 - @ModelAttribute 에노테이션과 SessionStatus
 */
package ex03;
...
public class Test28 {
    ...
    public String m1(Model model, HttpSession session) throws Exception{
        ...
        // => @SessionAttiributes에 지정된 이름이기 때문이다.
        model.addAttribute("name2", "홍길동"); 
        model.addAttribute("gender2", "남자");
        
        // 프론트 컨트롤러가 ServletRequest에 값 보관
        // => @SessionAttributes에 지정된 이름이 아니기 때문이다.
        model.addAttribute("tel2", "1111-1111");
        
        session.setAttribute("age2", "10"); 
    ...
    public String m2(
            ...
            // 파라미터 변수에 @ModelAttribute(이름) 에노테이션을 붙여라.
            //
    ...
    @ResponseBody
    public String m4(
            // 다른 페이지 컨트롤러의 @SessionAttributes에서 (Test27에 있는 값)
            // 지정된 값을 받을 때 ?
    ...
    public String m5(HttpSession session) throws Exception{
        // 다른 페이지 컨트롤러의 @SessionAttirbutes에서 (Test27에 있는 값)
    ...
    public String logout2(SessionStatus status) throws Exception{
        ...
        return String.format("@SessionAttributes 관리 대상을 제거하였다!");
    }
}
...
//     작업이 완료되어서 세션에 잠시 보관된 값들을 제거하고 싶다!" 라는 의미다.