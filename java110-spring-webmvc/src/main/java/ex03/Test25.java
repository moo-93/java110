...
public class Test25 {
    ...
    // @MatrixVaralbe 에노테이션을 사용하려면 스프링 설정에서 활성화 시켜야 한다.
    // 1) XML 설정
    //      <mvc:annotation-driven enable-matrix-variables="true"/>
    //
    // 2) javaConfig 설정
    //      @EnableWebMvc <=== 이 에노테이션을 추가한다.
    //      public class AppConfig{
    //          ...
    //      }
    // 테스트:
    //      ../search/car=sonata;color=red;auto=true;year=2013
    
    @RequestMapping(value="search2/{value}", produces="text/plain;charset=UTF-8")
    public String m2(
    ...
    // 테스트:
    //      ../search4/car/model=sonata;year=2013/tire/model=tt;year=2016
    @RequestMapping("search4/car/{value1}/tire/{value2}")
    public String m4(
    ...
    //                 {l3:[0-9][0-9]} 
    //                 {l4:[0-9][0-9]} 
    //                 {no:[0-9]+}_{yr:[0-9]+}v{ver:[a-z0-9]+} <== 숫자나 문자 한개이상
    
    @RequestMapping("search5/{l1:[0-9][0-9]}" + 
                            "{l2:[0-9][0-9]}" + 
                            "{l3:[0-9][0-9]}" + 
                            "{l4:[0-9][0-9]}" + 
