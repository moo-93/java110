...
public class Test22 {
    ...
    // 1) 리턴 타입이 String과 primitive type일 경우,
    //   => 문자열로 변환한 후 출력한다.
    // 2) 리턴 타입이 기타 객체일 경우,
    ...
    //   => "Http Message"는 클라이언트로 부터 받은 데이터,
    ...
    public Car m1() throws Exception{
        ...
        car.setMusics(new String[]{"오호롸!","소년점프","얍얍얍!"});
    ...
    // 프론트 컨트롤러는 리턴 값을 JSON으로 바꾸지 않고 그대로 출력한다.
    ...
    //    간주한다.
    //    변수=값&변수=값&... 형태의 데이터로 간주하고 Car 객체에 값을 넣으려고 할 것이다.
    //    그러나 클라이언트는 JSON 형식으로 데이터를 보내기 때문에 
    //    Car 객체에는 값이 들어가지 않는다.
    @RequestMapping("m8")
    ...
    // 프론트 컨트롤러는 페이지 컨ㅡ롤러가 작업한 겨로가를 담는 용도로 사용하는 줄 알고
    // 빈 맵 객체를 넘겨줄 것이다.
