...
public class Test20 {
    ...
    //  방법 1) DD 파일에 프론트 컨트롤러를 선언할때 다음의 태그를 설정해야 한다.
    //  <multipart-config>
    //      <max-file-size>2000000</max-file-size>
    //  </multipart-config>
    //
    //  방법 2) Apache common fileupload를 이용하여 멀티파트 데이터를 처리하는
    //  스프링 제공 객체를 추가하면 된다.
    //  => multipartResolver 라는 이름으로 다음과 같이 객체를 등록한다.
    //  => Apache commons-fileupload 의존 라이브러리를 추가해야 한다.
    //  => XML 설정 
    //  <beans:bean id="multipartResolver"
    //          class="org.springframework.web.multipart.commons.CommmonsMultipartResolver"
    ...
    //      CommonMultipartResolver multipartResolver = new CommonsMultipartResolver
    ...
    public String m1(
        ...
        for(MultipartFile f : files) {
            ...
            f.transferTo(new File(sc.getRealPath("/upload/" + filenames)));
