...
public class Test15 {
    ...
    // 클라이언트가 보낸 데이터는 메서드의 파라미터 타입에 맞춰서 
    // 변환하여 전달한다.
    ...
    // 클라이언트가 보낸 데이터를 request handler의 파라미터 타입으로 바꿔야 하는데
    // => 프론트 컨트롤러가 데이터 타입을 바꿀 대 WebDataBinder를 통해 설정된
    //    도구를 사용한다.
    // => @InitBinder는 프론트 컨트롤러가 WebDataBinder를 사용하기 전에
    //    이 에노테이션이 지정된 메서드를 호출하여 먼저 초기화시킨 다음에
    //    사용할 것을 요구하는 에노테이션이다.
    ...
    public void initBinder(WebDataBinder binder) {
        ...
        format.setLenient(false); //24시 기준으로 할건지 여부
        
        // String ===> java.util.Date 변환기 등록
        binder.registerCustomEditor(
                Date.class,
                //new DatePropertyEditor(format)); // 개발자가 만든것을 써도 되고
                new CustomDateEditor(format,true)); // 스프링에서 만든것을 써도 된다
