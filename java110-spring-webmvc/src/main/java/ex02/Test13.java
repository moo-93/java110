...
public class Test13 {
    ...
    // @RequestParam 으로 파라미터로 받을 클라이언트 값을 지정한다.
    // 기본은 필수이다. 따라서 값이 없으면 예외가 발생한다.
    @RequestMapping("m1")
    public String m1(
            @RequestParam(name="name") String name, // name value는 같다.
    ...
    // 오류가 발생한다.
    // => HTTP Stauts 400 - Bad Request 요류 발생
    @RequestMapping("m2")
    public String m2(
    ...
    // 값을 생략했을 때 기본 값을 같게 하고 싶다면 defaultValue 속성을 지정하라
    @RequestMapping("m3")
    public String m3(
            @RequestParam(name="name", required=false) String name, 
