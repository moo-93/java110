// 객체 생성 : 에노테이션으로 설정하기
// => XML <bean> 태그 대신 에노테이션을 이용하여 객체를 생성할 클래스를 지정할 수 있다.
//    클래스 선언부에 붙이면 된다.
// => 에노테이션
//     @Component
// => 스프링은 객체의 역할에 따라 구분할 수 있도록 다음의 에노테이션을 추가로 제공한다.
//     @Controller      : MVC 구조에서 컨트롤러 역할을 하는 클래스를 가리킨다.
//     @Service         : 비즈니스 로직을 수행하는 클래스를 가리킨다.
//     @Repository      : DAO처럼 데이터를 처리하는 클래스를 카리킨다.
//     @RestController  : RESTful 서비스를 처리하는 컨트롤러를 가리킨다.
// => 물론 @Autowired의 경우처럼
//    이 에노테이션을 처리할 객체를 스프링 IoC 컨테이너에 추가해야 한다.
