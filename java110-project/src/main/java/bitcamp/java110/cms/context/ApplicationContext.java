...
public class ApplicationContext {
    ...
    public ApplicationContext(String pakageName) throws Exception {
        ...
        // 패키지 폴더에 들어 있는 클래스를 찾아 클래스를 로딩 한 후 목록에 저장
        findClass(file, path);

        // 로딩된 클래스 목록을 뒤져서 @Component가 붙은 
        // 클래스에 대해 인스턴스를 생성하여 pool에 보관한다.
        createInstance();

        // objPool에 보관된 객체를 꺼내 @Autowired가 붙은 세터를 찾아 호출한다.
        // => 의존 객체 주입
        injectDependency();
    }
        ...
        for(File file : files) {
            if(file.isDirectory()) {
                ...
                try {
                    ...
                    classes.add(clazz); // 로딩한 클래스 정보를 목록에 보관
                } catch (Exception e) {}
            }
        }
    }
        ...
        for(Class<?> clazz : classes) {
            ...
            try {
                ...
                if(anno.value().length() > 0) {
                    // => Component 에너테이션 value 값으로 인스턴스를 objPool에 저장된다.
